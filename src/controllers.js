...
var CTRLS = {
  PID: {
    ...
    params: [{ id: 'Kp', l: 'Kp (angle)', min: 20, max: 140, s: 1, v: 60 }, { id: 'Kd', l: 'Kd (rate)', min: 5, max: 50, s: 0.5, v: 16 }, { id: 'Ki', l: 'Ki (integ)', min: 0, max: 8, s: 0.1, v: 2 }, { id: 'Kx', l: 'Kx (cart)', min: 0, max: 15, s: 0.5, v: 5 }, { id: 'Kv', l: 'Kv (vel)', min: 0, max: 20, s: 0.5, v: 8 }],
    make: function(p, sp) {
      var ie = 0;
      return function(s, dt) {
        ...
        if (Math.abs(rawF) < 19.5 || Math.sign(rawF) === Math.sign(s.th)) {
          ...
          satF = clamp(noITerm + p.Ki * ie, -20, 20);
        }
